...
export default function HomePage() {
    ...
    const [statusMessage, setStatusMessage] = useState('Ready to begin your next milking session.');
    ...
    const handlePauseResume = async () => {
        ...
        setIsPaused((paused) => {
            const next = !paused;
            setStatusMessage(next ? 'Session paused.' : 'Session resumed with music playing.');
            return next;
        });
    };
    ...
    const handleStop = () => {
        ...
        setStatusMessage('Session stopped. Save the milk quantity to record the session.');
    };
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const body: { data?: { id: string }; error?: { message?: string } } = await response.json();
            if (!response.ok) {
                throw new Error(body.error?.message || 'Unable to save the session');
            }

            setSaveSuccess('Milking session saved successfully.');
            setShowSaveForm(false);
            setMilkQuantity('');
            setStatusMessage('Session recorded. View history to check past milking data.');
        } catch (error) {
            ...
            <div className="page-card">
                <div className="header-row">
                    <div className="hero-copy">
                        ...
                        <p className="subtitle">Big buttons. Clear timer. Easy save.</p>
                        <div className="hero-stats">
                            ...
                            <div className="mini-stat">
                                ...
                                <span className="mini-stat-value">{formatSeconds(elapsedSeconds)}</span>
                                <span className="mini-stat-label">Timer</span>
                            </div>
                        </div>
                    </div>
                    <a href="/history" className="secondary-button">History</a>
                </div>
                ...
                <section className="feature-grid">
                    <div className="timer-panel">
                        ...
                        <div className="controls">
                            {!isRunning && !showSaveForm && (
                                <button className="primary-button action-button" onClick={handleStart}>
                            ...
                            {isRunning && (
                                <>
                                    <button className="secondary-button action-button" onClick={handlePauseResume}>
                                        <span className="action-symbol">{isPaused ? '>' : '||'}</span>
                                        <span>{isPaused ? 'Go' : 'Pause'}</span>
                                    </button>
                                    <button className="danger-button action-button" onClick={handleStop}>
                                        <span className="action-symbol">[]</span>
                                        <span>Stop</span>
                                    </button>
                                </>
                    ...
                    <aside className="side-panel">
                        <div className="visual-card">
                            <div className="visual-icon">MUSIC</div>
                            <div className="visual-text">
                                <strong>Sound</strong>
                                <span>Calm</span>
                            </div>
                        </div>
                        <div className="visual-card soil-card">
                ...
                {showSaveForm && (
                    <div className="input-card">
                        ...
                        <div className="input-row">
                            ...
                            />
                            <button className="primary-button action-button" onClick={handleSave}>
                                <span className="action-symbol">+</span>
                                <span>Save</span>
                            </button>
                        </div>
                ...
                <section className="insight-grid compact-grid">
                    <div className="compact-card">
                        ...
                        <span className="compact-label">Calm</span>
                    </div>
                    <div className="compact-card">
                        <span className="compact-icon">MILK</span>
                        <span className="compact-label">Record</span>
                    </div>
                    <div className="compact-card">
